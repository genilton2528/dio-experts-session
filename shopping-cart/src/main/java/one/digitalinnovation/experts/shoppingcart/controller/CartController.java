...
public class CartController {
    ...
    @GetMapping("/{id}")
    public Cart findById(@PathVariable Integer id) throws Exception {
        return cartRepository.findById(id).orElseThrow( () -> new Exception("Deu Ruim!") );
    }

    @DeleteMapping("/{id}")
    public void clear(@PathVariable Integer id) throws Exception {
        try {
